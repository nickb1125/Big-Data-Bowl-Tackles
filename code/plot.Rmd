...
plot_features <- function(game_id, play_id) {
    ...
    saveGIF({for (i in min(all_frames):max(all_frames)) {
      ...
      min_y = 0
      max_y = 54
      main <- ggplot() + geom_tile(data = data.frame(expand.grid(x=seq(0, 120), y=seq(0, 54))) %>% mutate(prob = 0), 
                                   aes(x = x, y = y, fill = prob), width = 10, height = 10)
      main <- main +
        scale_fill_gradient(low = "black", high = "brown1", limits = c(0, 10)) +
            geom_vline(aes(xintercept = seq(round_any(min_x, 5, f = floor), 
                                          round_any(max_x, 5, f = ceiling), by = 5)), 
                     color = 'white') +
          geom_segment(aes(x = seq(round_any(min_x, 5, f = floor), round_any(max_x, 5, f = ceiling), by = 1 ), 
      ...
      for (feature_name in unique(image_this_frame$feature_num))  {
        ...
        p <- ggplot() + ggtitle(feature_name) +
          ...
          scale_fill_gradient(low = "black", high = "dodgerblue2", limits = c(0, 0.05), oob = scales::squish)
        plot_list[[iter]] <- p
        iter = iter+1
      }
...
cumulative_contribution_df %>% left_join(eys_rank, on = c('nflId', 'displayName', 'position', 'final_cum_contribution'))  %>%
  ggplot() + 
  geom_line(aes(x = overall_frame_id, y = cum_contribution, group = nflId, alpha = best_or_worst)) +
...
cor(general_table$total_frame_num, general_table$avg_contribution)
```

```{r}
general_table %>% filter(displayName == "Chris Jones")
```
...
### SOI analysis 
```{r}
contribution_df %>% ggplot(aes(x = estimated_contribution, y = estimated_soi)) + geom_point()
```



```{r}
soi_df <- contribution_df %>%
  group_by(nflId, displayName, position, game_id, play_id) %>% 
  dplyr::summarise(max_soi= max(estimated_soi)) %>% ungroup() %>%
  left_join(
    tackles %>% dplyr::select(nflId, gameId, playId, tackle, assist, forcedFumble) %>% dplyr::rename(game_id = gameId, play_id = playId) %>%
     group_by(nflId, game_id, play_id) %>% dplyr::summarise(total_tackle_involvement = sum(tackle, assist, forcedFumble)) %>% ungroup(),
    on = c("nflId", "game_id", "play_id")) %>% mutate(total_tackle_involvement=ifelse(is.na(total_tackle_involvement), 0, total_tackle_involvement))

soi_df %>%ggplot+geom_point(aes(x = max_soi, y = total_tackle_involvement))
```



```{r}
contribution_df %>% filter(estimated_soi == min(estimated_soi))
```
