...
plot_marginal_contribution(game_id, play_id)
```
...
cumulative_contribution_df <-  contribution_df %>% arrange(play_id) %>% group_by(nflId, displayName, position) %>% 
  mutate(play_number = rank(play_id), cum_contribution = cumsum(estimated), 
         ...
         final_cum_contribution = cum_contribution[play_number == max(play_number)]) %>% ungroup()
...
cumulative_contribution_df  %>% dplyr::select(nflId, displayName, position, final_cum_contribution, avg_contribution) %>% distinct() %>%
  mutate(rank_cum_contribution = rank(-final_cum_contribution)) %>% 
  arrange(rank_cum_contribution)  %>% gt()
