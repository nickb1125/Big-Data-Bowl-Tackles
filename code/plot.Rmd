...
plot_marginal_contribution <- function(game_id, play_id) {
  pred_df <- read.csv(paste0("/Users/nickbachelder/Desktop/Personal Code/Kaggle/Tackles/", game_id, "_", play_id, ".csv")) %>% dplyr::select(-X) %>%
      mutate(x = as.numeric(x), y = as.numeric(y)) %>% filter(type == "expected")
  pred_no_original <- pred_df %>% filter(omit != 0)
  all_frames <- unique(pred_no_original$frameId)
  max_prob <- max(pred_no_original$prob)
  min_prob <- min(pred_no_original$prob)
  saveGIF({for (i in 24:max(all_frames)) {
    pred_df_frame <- pred_no_original %>% filter(frameId == i)
    p <- pred_df_frame %>% dplyr::select(-y) %>% group_by(omit, x) %>% dplyr::summarise(prob = sum(prob)) %>% ungroup() %>% mutate(omit = as.factor(omit)) %>%
      ggplot() + geom_line(aes(x = x, y = prob)) + facet_wrap(~omit, ncol = 1) + theme_void() +
