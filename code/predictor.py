...
colors = [(0, 0, 1), (0.7, 0.7, 0.7), (1, 0, 0)]  # Blue to Grey to Red
...
def update_main(frame_id, dataframe, scatter, ax):
    ...
    ax.scatter(tracking_now_off['x'], tracking_now_off['y'], np.max(z)+np.max(z)/100, c='grey', marker='o', label='Tracking Points')
    ax.scatter(tracking_now_def['x'], tracking_now_def['y'], np.max(z)+np.max(z)/100, c='dodgerblue', marker='o', label='Tracking Points')
    ax.scatter(tracking_now_ball['x'], tracking_now_ball['y'], np.max(z)+np.max(z)/100, c='blueviolet', marker='o', label='Tracking Points')
    ax.plot_surface(x_field, y_field, np.full_like(np.zeros((10,10)), np.max(z)), color="grey", alpha=0.5)
    ax.plot_surface(x_td1, y_td1, np.full_like(np.zeros((10,10)), np.max(z)), color="black", alpha=0.8)
    ax.plot_surface(x_td2, y_td2, np.full_like(np.zeros((10,10)), np.max(z)), color="black", alpha=0.8)

    ax.set_xlabel('Yardline')
    ax.set_title(f'Tackle Probability Density with Prediction Interval', fontweight='bold',)
...
def update(frame_id, dataframe, axs, omit_values):
    ...
    for ax, omit in zip(axs.flatten(), omit_values):
        ...
        fx = sorted(dataframe_now['x'].unique()) #[::5]
        fy = sorted(dataframe_now['y'].unique()) #[::5]
        z, zerror_lower, zerror_upper = [], [], []
        ...
        x, y = np.meshgrid(fx, fy)
        ax.plot_surface(x, y, np.array(z), cmap=custom_cmap_two_way, alpha=1, label=f'Omit={omit}', norm = Normalize(vmin=-max_z, vmax=max_z))
    
        ax.scatter(this_def_player['x'], this_def_player['y'], overall_max_z+overall_max_z/10000, c='dodgerblue', marker='o', label='Defense', alpha=1, s = 300)
        ax.scatter(tracking_now_ball['x'], tracking_now_ball['y'], overall_max_z+overall_max_z/10000, c='blueviolet', marker='o', label='Ball Carrier', alpha=1,s=300)
        ax.plot_surface(x_field, y_field, np.full_like(np.zeros((10,10)), overall_max_z), color="grey", alpha=0.5)
        ...
        ax.set_title(f'{player_name}', fontsize=40, pad=20, loc='center', y=0.1)
...
if contribution_plot:
    ...
    play_object = play(game_id, play_id, tracking)
    def_df = play_object.refine_tracking(frame_id = play_object.min_frame)["Defense"]
    omit_values = def_df.nflId.unique()

    fig, axs = plt.subplots(len(omit_values)//4, 4,  figsize=(50, 20), subplot_kw={'projection': '3d'}, constrained_layout=True)
    axs = axs.flatten()
