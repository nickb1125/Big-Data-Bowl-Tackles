...
ggplot() + geom_line(data = loss_df, aes(x = frames_from_eop, y = test_loss, group = bag), alpha = 0.1) +
  geom_line(data = loss_df %>% filter(frames_from_eop, bag == "overall"), 
                         aes(group = frames_from_eop, y = test_loss)) + theme_classic() +
  xlab("Frames from End of Play") + ylab("Negative Log Liklihood Loss")
